...
class index:
    def GET(self):
        i =web.input()
        return self.get_down_detail(i['q'])

    def get_down_detail(self, url):
        s = requests.session()
        ...
        return '"' + r1_json['result']['title'] + '" ' + r1_json['result']['serverUrl'].replace('http', 'https') + '/download?file=' + r1_json['result']['id_process']
